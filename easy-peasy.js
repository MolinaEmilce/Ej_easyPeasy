...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			...
			///////////////////////////////////////////////////////
			let titulos = [];
			//recorre el objeto y los lamacena en titulos como array
			movies.forEach(function(element){
				titulos.push(element.title);
			})
			titulos.sort();//el meotodo sort ordena alfabeticamente los datos de la variable titulo, solo en array
			titulos.forEach(element => {
				res.write(`Titulo : ${element} \n`);
			});
		...
		case '/en-cartelera':
            ...
            movies.forEach(element => {
                res.write(` \n Nombre: ${element.title.toUpperCase()} \n Reseña: ${element.overview}
        ...
        let votadas = movies.filter(function(elemento){//variable que recibe las peliculas filtradas segun la cantidad de votos
			return elemento.vote_average >= 7;
		});
		votadas.sort(function(a,b){
			return( (a.title < b.title ? -1 : ( (a.title > b.title) ? 1 : 0))  );
		});	
		res.write(`Las mas votadas son : ${votadas.length} \n\n`);
		//////////////////////////////////
		let promedio = 0;
		...
		case '/preguntas-frecuentes':
            ...
            res.write(`
                Pregunta: ${element.faq_title} \n Respuesta: ${element.faq_answer}
                
                
                `);
            });
			res.end('Preguntas Frecuentes.Fin');
			break;
		default:
